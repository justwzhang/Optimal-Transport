#ifndef _DARTLIB_POLYGON_3D_H_
#define _DARTLIB_POLYGON_3D_H_
...
class CPolygon3D
{
  public:
    ...
    CSphericalPolygon spherical_project();

    /*! the 3D plane of the polygon3D */
    CPlane3D plane()
    {
        assert(m_edges.size() > 2);
        CPoint a = m_edges[0].start();
        CPoint b = m_edges[1].start();
        CPoint c = m_edges[2].start();

        return CPlane3D(a, b, c);
    };
...
inline CSphericalPolygon CPolygon3D::spherical_project()
{
    CSphericalPolygon poly;
...
/*!
 *	transform a 3D polygon to the plane ordinates
 */

inline CPolygon3D transform(CPlane3D& plane, CPolygon3D poly)
{
    CPolygon3D Q;
    ...
    {
        CSegment3D& s = poly.edges()[i];
        pts.push_back(s.start());
    }
    std::vector<CPoint> qts;
    for (int i = 0; i < pts.size(); i++)
    {
        CPoint q = plane.transform(pts[i]);
        qts.push_back(q);
    }

    for (int i = 0; i < qts.size(); i++)
    {
        CSegment3D s(qts[i], qts[(i + 1) % qts.size()]);
        Q.add(s);
    }
...
void plane_cylinder_intersection(CPlane3D& plane, CPolygon2D& cpoly, CPolygon3D& Q)
{
    std::vector<CPoint2> pts;
    for (int i = 0; i < cpoly.edges().size(); i++)
    {
        CSegment2D& s = cpoly.edges()[i];
        pts.push_back(s.start());
    }
    ...
    for (int i = 0; i < pts.size(); i++)
    {
        CPoint2 p = pts[i];
        double z = plane.z(p);
        qts.push_back(CPoint(p[0], p[1], z));
    }

    for (int i = 0; i < qts.size(); i++)
    {
        CSegment3D seg(qts[i], qts[(i + 1) % qts.size()]);
        Q.add(seg);
    }
};
...
CPolygon3D inverse_transform(CPlane3D& plane, CPolygon3D& cpoly)
{
    CPolygon3D Q;
    ...
    {
        CPoint p = pts[i];
        CPoint q = plane.inverse_transform(p);
        qts.push_back(q);
    }
...
// plane, convert a planar polygon on the plane to a spacial polygon

CPolygon3D inverse_transform(CPlane3D& plane, CPolygon2D& cpoly)
{
    CPolygon3D Q;
