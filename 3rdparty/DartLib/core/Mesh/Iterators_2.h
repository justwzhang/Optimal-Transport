...
class VertexInDartIterator
{
  public:
    VertexInDartIterator(typename M::CVertex* pV)
    {
        ...
        // iterate the darts ccwly.
        std::reverse(m_darts.begin(), m_darts.end());
...
template <typename M>
class VertexOutDartIterator
{
  public:
    VertexOutDartIterator(typename M::CVertex* pV)
    {
        typename M::CDart* pD0 = (typename M::CDart*) pV->dart()->beta(1);
        typename M::CDart* pD = pD0;
        ...
        do
        {
            ...
            if (pD->beta(2) != NULL)
                pD = (typename M::CDart*) pD->beta(2)->beta(1);
            else
                break;
        } while (pD != pD0);
