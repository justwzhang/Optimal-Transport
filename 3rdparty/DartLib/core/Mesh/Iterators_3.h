...
class VolumeIterator
{
  ...
  private:
    M* m_pMesh;
    typename std::list<typename M::CVolume*>::iterator m_iter;
};
...
{
  public:
    VertexFaceIterator(M* pMesh, typename M::CVertex* pV)
    {
        m_pMesh = pMesh;
        ...
        for (typename M::CDart* d : darts)
            m_faces.insert((typename M::CFace*) d->cell(2));
...
{
  public:
    EdgeFaceIterator(typename M::CEdge* pE, typename M::CVertex* pTarget = NULL)
    {
        ...
        if (pTarget && pTarget != (typename M::CVertex*) pD0->cell(0))
            std::reverse(m_faces.begin(), m_faces.end());
...
{
  public:
    EdgeVolumeIterator(typename M::CEdge* pE, typename M::CVertex* pTarget = NULL)
    {
        // If pE lies on boundary, then the dart on the edge
        // must lies on boundary too.
        typename M::CDart* pD0 = (typename M::CDart*) pE->dart();
        ...
        do
        {
            m_volumes.push_back((typename M::CVolume*) pD->cell(3));
...
{
  public:
    VolumeFaceIterator(typename M::CVolume* pVol)
    {
        typename M::CDart* pD = (typename M::CDart*) pVol->dart();
        m_faces.push_back((typename M::CFace*) pD->cell(2));

        // TODO: The following implementation is effcient but only work on tet mesh.
        int orbit[8] = {1, 1, 2, 1, 1, 2, 1, 2};
        int  mask[8] = {0, 0, 1, 0, 0, 1, 0, 1};
        for (int i = 0; i < 8; ++i)
        {
            pD = (typename M::CDart*) pD->beta(orbit[i]);
            if (mask[i] == 1)
...
template <typename M>
class VolumeEdgeIterator
{
  public:
    VolumeEdgeIterator(typename M::CVolume* pVol)
    {
        typename M::CDart* pD = (typename M::CDart*) pVol->dart();
        m_edges.push_back((typename M::CEdge*) pD->cell(1));

        // TODO: The following implementation is effcient but only work on tet mesh.
        int orbit[7] = {1, 1, 2, 1, 1, 2, 1};
        int  mask[7] = {1, 1, 0, 1, 1, 0, 1};
        for (int i = 0; i < 7; ++i)
        {
            pD = (typename M::CDart*) pD->beta(orbit[i]);
            if (mask[i] == 1)
