...
class TBaseMesh_2
{
  public:
    ...
    using VertexOutDartIterator= Dim2::VertexOutDartIterator<T_BASEMESH>;
    ...
    using FaceDartIterator     = Dim2::FaceDartIterator    <T_BASEMESH>;
    ...
    /*!
     *  Disable copy constructor function
     */
    TBaseMesh_2(const T_BASEMESH& mesh) = delete;

    /*!
     *  Disable assignment operator
     */
    T_BASEMESH& operator = (const T_BASEMESH& mesh) = delete;

    /*!
     *  Copy method
     *  \param pMesh: the to be copied mesh pointer. 
     */
    void copy(T_BASEMESH * pSrcMesh);
     ...
     *  \param    faces: the indices of points to construct faces
     */
    void load(const std::vector<CPoint>            & vertices, 
              const std::vector< std::vector<int> >& facess);
   ...
   */
    ...
    CFace*   id_face  (int id) { return m_map_face.find(id)   == m_map_face.end()   ? NULL : m_map_face[id];  };
    ...
    CPoint calc_face_normal(CFace* face, bool is_unit = true);
    double calc_face_area(CFace* face);
    ...
    typedef VertexOutDartIterator VertexOutHalfEdgeIterator;
    ...
    size_t numFaces()    { return num_faces();    };
    size_t numHalfedges(){ return num_darts();    };
...
T_TYPENAME
void T_BASEMESH::copy(T_BASEMESH* rhs)
{
    if (this == rhs) return;

    // 1. prepare the data
    std::map<int, CPoint>           vert_id_point;
    std::map<int, std::vector<int>> face_id_vids;

    //T_BASEMESH* prhs = &rhs;
    for (VertexIterator viter(rhs); !viter.end(); ++viter)
    {
        CVertex* pV = *viter;
        int     vid = pV->id();
        CPoint&   p = pV->point();
    ...
    for (FaceIterator fiter(rhs); !fiter.end(); ++fiter)
    {
        CFace* pF = *fiter;
        int   fid = pF->id();
        ...
        {
            vert_ids.push_back((*fviter)->id());
        }

        face_id_vids.insert(std::make_pair(fid, vert_ids));
    }

    // 2. load the data, and construct the initial mesh
    this->load(vert_id_point, face_id_vids);

    // 3. write the attributes
    for (VertexIterator viter(rhs); !viter.end(); ++viter)
        (*viter)->to_string();
    for (EdgeIterator eiter(rhs); !eiter.end(); ++eiter)
        (*eiter)->to_string();
    for (FaceIterator fiter(rhs); !fiter.end(); ++fiter)
        (*fiter)->to_string();
    for (DartIterator diter(rhs); !diter.end(); ++diter)
        (*diter)->to_string();

    // 4. load the attributes
    for (VertexIterator viter(this); !viter.end(); ++viter)
    {
        CVertex*  pV = *viter;
        pV->string() = rhs->id_vertex(pV->id())->string();
        pV->from_string();
    }

    for (EdgeIterator eiter(this); !eiter.end(); ++eiter)
    {
        CEdge  *  pE = *eiter;
        CVertex*  pA = edge_vertex(pE, 0);
        CVertex*  pB = edge_vertex(pE, 1);

        CVertex* _pA = rhs->id_vertex(pA->id());
        CVertex* _pB = rhs->id_vertex(pB->id());
        CEdge  * _pE = rhs->vertex_edge(_pA, _pB);

        pE->string() = _pE->string();
        pE->from_string();
    }
    ...
    {
        CFace*    pF = *fiter;
        pF->string() = rhs->id_face(pF->id())->string();
        pF->from_string();
    }

    for (DartIterator diter(this); !diter.end(); ++diter)
    {
        CDart  *  pD = *diter;
        CVertex*  pV = C0(pD);
        CFace  *  pF = C2(pD);

        CVertex* _pV = rhs->id_vertex(pV->id());
        CFace  * _pF = rhs->id_face(pF->id());
        CDart  * _pD = rhs->dart(_pV, _pF);

        pD->string() = _pD->string();
        pD->from_string();
    }
}
    ...
    const std::vector<std::vector<int>>& facess)
{
    ...
    for (int i = 0; i < facess.size(); ++i)
    {
        ...
        for (int j = 0; j < vids.size(); ++j)
            vids[j] += 1;
        face_id_vids.insert(std::make_pair(i + 1, vids));
    }

    load(vert_id_point, face_id_vids);
}
...
{
    // vert is the target of the dart
    CDart* d = D(face);
    while (true)
    {
        if (C0(d) != vert)
...
T_TYPENAME
CPoint T_BASEMESH::calc_face_normal(CFace* face, bool is_unit)
{
    // TODO: To be more robust, we can also choose the following three points
    //         if the polygon is not a triangle.
    //  V_i: one point on the vertex
    //  V_j: another point far away from V_i, s.t. (V_i - V_j).norm() > 1e-12
    //    C: barycenter of the polygon

    std::vector<CPoint> pts;
    for (FaceVertexIterator fvit(face); !fvit.end(); ++fvit)
    {
        CPoint& p = (*fvit)->point();
        if (pts.size() > 0 && (pts.back() - p).norm() < 1e-12) 
            continue;
        pts.push_back(p);
        if (pts.size() >= 3)
            break;
    ...
    CPoint fn = (pts[1] - pts[0]) ^ (pts[2] - pts[0]);
    if (is_unit) fn /= fn.norm();

    assert(!std::isnan(fn[0]));
    return fn;
}

T_TYPENAME
double T_BASEMESH::calc_face_area(CFace* face) 
{
    double area = 0.0;
    // assume the face is a planar convex polygon.
    std::vector<CPoint> pts;
    for (FaceVertexIterator fvit(face); !fvit.end(); ++fvit)
    {
        CPoint& p = (*fvit)->point();
